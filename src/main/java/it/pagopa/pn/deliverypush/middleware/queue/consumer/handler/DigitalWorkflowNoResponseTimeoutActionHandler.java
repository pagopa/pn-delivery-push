...
@Component
@CustomLog
public class DigitalWorkflowNoResponseTimeoutActionHandler extends AbstractActionEventHandler {
    private final DigitalWorkFlowRetryHandler digitalWorkFlowRetryHandler;

    public DigitalWorkflowNoResponseTimeoutActionHandler(TimelineUtils timelineUtils, DigitalWorkFlowRetryHandler digitalWorkFlowRetryHandler) {
        super(timelineUtils);
    ...
    public SupportedEventType getSupportedEventType() {
        return SupportedEventType.DIGITAL_WORKFLOW_NO_RESPONSE_TIMEOUT_ACTION;
    }
    ...
    public void handle(Action action, MessageHeaders headers) {
        ...
        try {
            log.debug("Handle action of type DIGITAL_WORKFLOW_NO_RESPONSE_TIMEOUT_ACTION, with payload {}", action);

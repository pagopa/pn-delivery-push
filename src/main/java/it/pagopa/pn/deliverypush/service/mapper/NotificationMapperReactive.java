...
import it.pagopa.pn.delivery.generated.openapi.clients.delivery_reactive.model.*;
...
public class NotificationMapperReactive {
    private NotificationMapperReactive(){}
    ...
    private static List<NotificationRecipientInt> mapNotificationRecipient(List<NotificationRecipient> recipients) {
        ...
        for (NotificationRecipient recipient : recipients){
            NotificationRecipientInt recipientInt = RecipientMapperReactive.externalToInternal(recipient);
    ...
    public static SentNotification internalToExternal(NotificationInt notification) {
        ...
        List<NotificationRecipient> recipients = notification.getRecipients().stream()
                .map(RecipientMapperReactive::internalToExternal).collect(Collectors.toList());

        sentNotification.setRecipients(recipients);

        List<NotificationDocument> documents = notification.getDocuments().stream().map(
                NotificationMapperReactive::getNotificationDocument).collect(Collectors.toList());

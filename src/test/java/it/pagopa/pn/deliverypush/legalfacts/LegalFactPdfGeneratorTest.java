...
class LegalFactPdfGeneratorTest {
        ...
        @Test
        @ExtendWith(SpringExtension.class)
        void generateNotificationAAR_RADD_ALT_Test_bilingual() {
                Mockito.when(pnSendModeUtils.getPnSendMode(Mockito.any())).thenReturn(PnSendMode.builder()
                        .aarTemplateTypeChooseStrategy(new StaticAarTemplateChooseStrategy(AarTemplateType.AAR_NOTIFICATION_RADD_ALT_bilingual))
                        .build());

                Path filePath = Paths.get(TEST_DIR_NAME + File.separator + "test_NotificationAAR_RADDalt_bilingual.pdf");
                NotificationSenderInt notificationSenderInt = NotificationSenderInt.builder()
                        .paId("TEST_PA_ID")
                        .paTaxId("TEST_TAX_ID")
                        .paDenomination("Lorem ipsum dolor sit amet, consectetur adipiscing elit. Pellentesque porttitore")
